defmodule DemoWeb.Live.Product.PaginationLiveTest do
  use DemoWeb.ConnCase, async: false

  import Demo.EctoFactory

  # Products use default per_page_default: 15, per_page_options: [15, 50, 100]

  describe "pagination" do
    test "paginates items with default per_page", %{conn: conn} do
      insert_list(20, :product)
  ...
  describe "ordering preserves pagination" do
    test "ordering resets to page 1", %{conn: conn} do
      insert_list(20, :product)

      params = %{"page" => "2", "order_by" => "name", "order_direction" => "asc"}

      conn
      |> visit(~p"/admin/products?#{params}")
      |> assert_has("div", text: "(20 total)")
    end
  end
end
