...
Backpex checks this twice: when the modal is opened (the button is not rendered at all when the check fails, and the route itself refuses) and again when the form is submitted, so a permission revoked while the form was open cannot be used. An unauthorized submit raises `Backpex.ForbiddenError`, which crashes the LiveView and reloads the client — the write does not happen, and the user is not shown a 403 page or a message. See [Authorization](live-resource-authorization.html) for the details.
